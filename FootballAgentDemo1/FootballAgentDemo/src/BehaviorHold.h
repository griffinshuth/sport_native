...
public:
	...
	void SubmitVisualRequest(const ActiveBehavior & hold, double plus = 0.0) {
		...
		mAgent.getClient()->getVisualSystem()->RaisePlayer(mAgent, -hold.mKeyOppGB.mUnum, 2.0 + plus);
		mAgent.getClient()->getVisualSystem()->RaisePlayer(mAgent, -hold.mKeyOppGT.mUnum, 2.0 + plus);
	}
