...
bool BehaviorHoldExecuter::Execute(const ActiveBehavior & hold)
{
	...
	if(hold.mDetailType == BDT_Hold_Turn){
		return mAgent.getClient()->getDasher()->GetTurnBodyToAngleAction(mAgent, hold.mAngle).Execute(mAgent);
	}
	else
		return mAgent.getClient()->getKicker()->KickBallCloseToBody(mAgent ,hold.mAngle, 0.6);
}
