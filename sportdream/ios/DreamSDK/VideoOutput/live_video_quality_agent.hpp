...
#ifndef live_video_quality_agent_hpp
#define live_video_quality_agent_hpp

#include <stdio.h>
#include "platform_4_live_common.h"
#include "gearshift_box.hpp"
#include <vector>
#include <string>
#include <sstream>
#include <vector>
...
private:
  ...
  const int STRATEGY_REDUCTION_RISE_SMART_CIRCLE = 4;
  ...
  std::map<std::string, WriteTimeInfo*> writeTimeAccumulate;
  ...
  bool dealBaseLine(int avgSendBitrate,std::map<std::string, WriteTimeInfo*> writeTimeMapping);
  
public:
  static VideoQualityAgent* GetInstance();
  ...
  void bitrateFeedback(int sendAVGBitrate, int compressedAVG, float queueState ,std::map<std::string, WriteTimeInfo*> writeTimeMapping);
  
  void adjustmentBitrate(int adaptBitrate, int mode);
  
  void adjustmentBitrateAndFps(int adaptBitrate,int fps);
  
  int getBitrate();
};

#endif /* live_video_quality_agent_hpp */
