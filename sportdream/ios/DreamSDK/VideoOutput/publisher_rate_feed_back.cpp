...
PublisherRateFeedback::PublisherRateFeedback() {
  ...
  writeTimeMapping[kRTT_0_30] = new WriteTimeInfo();
  writeTimeMapping[kRTT_30_100] = new WriteTimeInfo();
  writeTimeMapping[kRTT_100_200] = new WriteTimeInfo();
  writeTimeMapping[kRTT_200_300] = new WriteTimeInfo();
  writeTimeMapping[kRTT_300_1000] = new WriteTimeInfo();
  writeTimeMapping[kRTT_1000Plus] = new WriteTimeInfo();
}
...
VideoQualityAgent* PublisherRateFeedback::getQualityAgent(){
  return qualityAgent;
}
...
void PublisherRateFeedback::publishPktSuccess(int pktSize,int writeTimeInterval) {
  sendSizeInWindow += pktSize;
  if (writeTimeInterval<30) {
    this->writeTimeMapping[kRTT_0_30]->count += 1;
    this->writeTimeMapping[kRTT_0_30]->totalms += writeTimeInterval;
  }
  else if (writeTimeInterval>=30 && writeTimeInterval<100) {
    this->writeTimeMapping[kRTT_30_100]->count += 1;
    this->writeTimeMapping[kRTT_30_100]->totalms += writeTimeInterval;
  }
  else if (writeTimeInterval>=100 && writeTimeInterval<200) {
    this->writeTimeMapping[kRTT_100_200]->count += 1;
    this->writeTimeMapping[kRTT_100_200]->totalms += writeTimeInterval;
  }
  else if (writeTimeInterval>=200 && writeTimeInterval<300) {
    this->writeTimeMapping[kRTT_200_300]->count += 1;
    this->writeTimeMapping[kRTT_200_300]->totalms += writeTimeInterval;
  }
  else if (writeTimeInterval>=300 && writeTimeInterval<1000) {
    this->writeTimeMapping[kRTT_300_1000]->count += 1;
    this->writeTimeMapping[kRTT_300_1000]->totalms += writeTimeInterval;
  }
  else if (writeTimeInterval>=1000) {
    this->writeTimeMapping[kRTT_1000Plus]->count += 1;
    this->writeTimeMapping[kRTT_1000Plus]->totalms += writeTimeInterval;
  }
  //    LOGI("Verbose : write_frame time interval: %d",writeTimeInterval);
}
...
bool PublisherRateFeedback::detectNotifyEncoderReConfigure() {
  ...
  if ((currentTimeMills - latestNotifyTimeMills) > encoderReconfigInterval) {
    ...
    qualityAgent->bitrateFeedback(sendAVGBitrate, compressAVGBitrate, videoPacketQueueState,writeTimeMapping);
    WriteTimeMappingHelper::reset(this->writeTimeMapping);
  }
  return ret;
}
