...
- (void)udpSocket:(GCDAsyncUdpSocket *)sock didReceiveData:(NSData *)data fromAddress:(NSData *)address withFilterContext:(id)filterContext
{
  ...
  if(_isServer){
    ...
    if([aStr isEqualToString:@"hand"]){
      ...
      [self.delegate broadcastReceived:ip];
    ...
    if([aStr isEqualToString:@"echo"]){
      ...
      [self.delegate serverDiscovered:serverIP];
    }
  }
}
...
- (void)socket:(GCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port
{
  if(sock == androidTempTcpSocket){
    ...
    if(!_isServer){
      [self.delegate clientSocketConnected];
...
- (void)socket:(GCDAsyncSocket *)sock didAcceptNewSocket:(GCDAsyncSocket *)newSocket
{
  ...
  [self.delegate acceptNewSocket:newSocket];
}
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
  if(_isServer){
    ...
    while(packet){
      ...
      [self.delegate serverReceiveData:ID data:contentData sock:sock];
      //处理完毕
      last = packet;
    ...
    while(packet){
      ...
      NSLog(@"packet ID:%d",ID);
      NSData *contentData = [[NSData alloc] initWithBytes:content length:len];
      [self.delegate clientReceiveData:ID data:contentData];
      //处理完毕
      last = packet;
...
- (void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(NSError *)err
{
  if(androidTempTcpSocket == sock){
    ...
    if(_isServer){
      if(tcpSocket != sock){
        [self.delegate serverSocketDisconnect:sock];
      }
    }else{
        [self.delegate clientSocketDisconnect:sock];
    }
  }
