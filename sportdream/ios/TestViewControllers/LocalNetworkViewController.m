...
@property (nonatomic,strong) LocalWifiNetwork*  testNetwork;
@end
...
- (void)viewDidLoad {
  ...
  UIButton* clientButton = Button.fnt(@15).color(@"#0065F7").border(1, @"#0065F7").borderRadius(3).onClick(^(UIButton* btn){
    self.testNetwork = [[LocalWifiNetwork alloc] initWithType:false];
    self.testNetwork.delegate = self;
    [self.testNetwork searchDirectorServer];
  });
  ...
  UIButton* serverButton = Button.fnt(@15).color(@"#0065F7").border(1, @"#0065F7").borderRadius(3).onClick(^(UIButton* btn){
    self.testNetwork = [[LocalWifiNetwork alloc] initWithType:true];
    self.testNetwork.delegate = self;
  });
  ...
  UIButton* testpacket = Button.fnt(@15).color(@"#0065F7").border(1, @"#0065F7").borderRadius(3).onClick(^(UIButton* btn){
    ...
    [self.testNetwork clientSendPacket:1 data:info];
  });
...
-(void)serverReceiveData:(LocalWifiNetwork*)network packetID:(uint16_t)packetID data:(NSData*)data sock:(GCDAsyncSocket *)sock
{
  ...
  if(packetID == 1){
    ...
    NSData *info =[welcomeMsg dataUsingEncoding:NSUTF8StringEncoding];
    [self.testNetwork serverSendPacket:2 data:info sock:sock];
  }
}
