...
- (void)viewDidLoad {
  ...
  {
    [filesLayout addSubview:[self createLocalCameraView:filesactions[i] index:i]];
  }
...
{
  [self.network serverSendPacket:packetID data:data sock:sock];
}
...
-(void)dataFromPostProgress:(NSData*)yuvData frameTime:(CMTime)frameTime
{
  ...
  [self.camera pushExternalVideoData:pixelBuf timeStamp:timestamp];
}
...
-(MyLinearLayout*)createLocalCameraView:(NSString*)name index:(NSInteger)index
{
  ...
  UIButton* smallH264Button = Button.fnt(@15).color(@"#0065F7").border(1, @"#0065F7").borderRadius(3);
