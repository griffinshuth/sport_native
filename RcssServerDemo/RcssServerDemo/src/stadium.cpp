...
showinfo_double Stadium::getCurrentInfo()
{
    showinfo_double show;
    show.pmode = static_cast< char >( this->playmode() );
    if ( ! this->teamLeft().name().empty() )
    {
        int n = (int)std::min( this->teamLeft().name().length(), size_t( 16 ) );
        std::strncpy( show.team[0].name,
                     this->teamLeft().name().c_str(),
                      n);
        if(n<16)
            show.team[0].name[n] = '\0';
        else
            show.team[0].name[15] = '\0';
        show.team[0].score = this->teamLeft().point();
    }
    else
    {
        std::strcpy( show.team[0].name, "" );
        show.team[0].score = 0;
    }
    
    if ( ! this->teamRight().name().empty() )
    {
        int n = (int)std::min( this->teamRight().name().length(), size_t( 16 ) );
        std::strncpy( show.team[1].name,
                     this->teamRight().name().c_str(),
                      n);
        if(n<16)
            show.team[1].name[n] = '\0';
        else
            show.team[1].name[15] = '\0';
        show.team[1].score =  this->teamRight().point();
    }
    else
    {
        std::strcpy( show.team[1].name, "" );
        show.team[1].score =  0;
    }
    
    show.pos[0].x =  this->ball().pos().x;
    show.pos[0].y =  this->ball().pos().y;
    
    const Stadium::PlayerCont & players = this->players();
    for ( int i = 0; i < MAX_PLAYER * 2; ++i )
    {
        show.pos[i+1].enable =  players[i]->state() ;
        double player_x = players[i]->pos().x;
        double player_y = players[i]->pos().y;
        show.pos[i+1].x =  player_x;
        show.pos[i+1].y =  player_y;
        show.pos[i+1].angle =  (Int16)Rad2IDegRound( players[i]->angleBodyCommitted() ) ;
        show.pos[i+1].side =  players[i]->side() ;
        show.pos[i+1].unum =  players[i]->unum() ;
    }
    show.time =  this->time();
